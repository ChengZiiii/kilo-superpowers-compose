...
// ─── needsSetup：版本标记比对 ──────────────────────────────────────────
test('needsSetup: 标记缺失 → true', () => {
  const home = mkTempHome();
  try {
    const mf = path.join(home, 'marker');
    assert.equal(P.needsSetup(mf), true);
  } finally {
    rmrf(home);
  }
});

test('needsSetup: 版本不符 → true；版本一致 → false', () => {
  const home = mkTempHome();
  try {
    const mf = path.join(home, 'marker');
    fs.writeFileSync(mf, '0.0.0', 'utf8');
    assert.equal(P.needsSetup(mf), true);
    // 写入与当前包版本一致 → false
    const pkgVersion = JSON.parse(
      fs.readFileSync(path.resolve('package.json'), 'utf8')
    ).version;
    fs.writeFileSync(mf, pkgVersion, 'utf8');
    assert.equal(P.needsSetup(mf), false);
  } finally {
    rmrf(home);
  }
});
...
// ─── runSetup：复制命令 + 写标记 ───────────────────────────────────────
test('runSetup: 复制 superpowers.md 到目标并写标记', () => {
  const home = mkTempHome();
  try {
    const commandsSrc = path.join(home, 'pkg-commands');
    const commandsDst = path.join(home, 'config', 'commands');
    const mf = path.join(home, 'config', 'marker');
    fs.mkdirSync(commandsSrc, { recursive: true });
    fs.writeFileSync(path.join(commandsSrc, 'superpowers.md'), '# /superpowers\n', 'utf8');

    P.runSetup({ commandsSrc, commandsDstDir: commandsDst, markerFile: mf });

    assert.ok(fs.existsSync(path.join(commandsDst, 'superpowers.md')), '命令应被复制');
    assert.ok(fs.existsSync(mf), '标记应被写入');
    const pkgVersion = JSON.parse(
      fs.readFileSync(path.resolve('package.json'), 'utf8')
    ).version;
    assert.equal(fs.readFileSync(mf, 'utf8').trim(), pkgVersion);
  } finally {
    rmrf(home);
  }
});

test('runSetup: 源命令缺失不抛异常（仅写标记）', () => {
  const home = mkTempHome();
  try {
    const commandsSrc = path.join(home, 'nope'); // 不存在
    const commandsDst = path.join(home, 'config', 'commands');
    const mf = path.join(home, 'config', 'marker');
    // 不应抛
    P.runSetup({ commandsSrc, commandsDstDir: commandsDst, markerFile: mf });
    assert.ok(!fs.existsSync(path.join(commandsDst, 'superpowers.md')));
    assert.ok(fs.existsSync(mf), '标记仍应被写入');
  } finally {
    rmrf(home);
  }
});
...
test('plugin(): config 钩子注入 3 个 agent 与 skills.paths，并复制命令 + 写标记', async () => {
  const home = mkTempHome();
...
test('plugin(): 命令复制 + 标记写入发生在隔离 KILO_HOME 下', async () => {
  const home = mkTempHome();
  const restore = withKiloHome(home);
  try {
    await P.plugin();
    ...
    assert.ok(fs.existsSync(cmdDst), '/superpowers 命令应被复制到隔离配置目录');
    assert.ok(fs.existsSync(marker), '版本标记应被写入隔离配置目录');
  } finally {
    restore();
    rmrf(home);
  }
