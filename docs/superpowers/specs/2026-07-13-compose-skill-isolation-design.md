...
## 13. 关联决策：计划获批后的执行方式 handoff（文字检查点）

与隔离机制一并落地的一个 compose 工作流增强：复杂路径"计划获批"后插入一步
**执行方式检查点**（文字），向用户呈现 A/B/C 三选一（子代理驱动 / 移交新会话 /
本会话内执行），不默认直奔子代理。

**为何不用 kilo 原生 `plan_exit` 弹窗：** 已核实原生 `plan_exit` 可触发
"Ready to implement?"三选项弹窗（compose 加 `plan_exit: allow` 即可触发），但其中
"Start new session" 在 `startNew()` 里**硬编码注入 `agent:"code"` 并立即 `loop()` 开跑**
（`packages/opencode/src/kilocode/plan-followup.ts`）。不改 kilo 核心无法把它改成
compose——无 config / env / 插件钩子可覆盖；`default_agent` 也无效（自动注入的实现
指令锁死 code，`default_agent` 只影响新会话里之后的手动消息）。原生弹窗为 plan→code
直接实现而设计，与 compose 的子代理编排模型不搭。

故选**文字检查点**：compose 自己呈现选择；用户选"移交新会话"时，由 compose 生成
自包含 bootstrap 提示词（计划路径 + spec 路径 +"从 Task 1 起，加载
compose-executing-plans"），用户在**新的 compose 会话**里粘贴执行——无 code 绕道、
上下文干净。详见实现计划 Task 2b。
